...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if (!$existEmail) {
            $_SESSION["error"] = "Cet e-mail n'existe pas.";
            $error = isset($_SESSION["error"]) ? $_SESSION["error"] : '';
        } else {
            // appel de la fonction pour supprimer de la table
            $model->delete($email);
            $model->sendEmail2($email);
            $_SESSION['valid'] = "Vous vous êtes bien désinscrit";
...
<body>
    ...
    <nav>
        <ul>
            ...
            <li><a href="http://localhost/wordpress1/boutique/">Boutique</a></li>
    ...
    <form method="post" action="">
        ...
        <div>
            ...
            <input type="text" name="nom" id="nom" placeholder="Entrez votre nom" pattern="[A-Za-z\s]+" title="Veuillez entrer uniquement du texte">
        </div>
        <div>
